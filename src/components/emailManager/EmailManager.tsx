...
const EmailManager: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'personal' | 'work' | 'temporary' | string>('personal');
  const [categories, setCategories] = useState(['personal', 'work', 'temporary']);
  ...
  const personalItems = [
    {
      ...
      usage: 'Used for daily life. Contains subscription usage for YouTube Premium, ChatGPT Plus...',
    },
    {
          ...
          {categories.map((category) => (
            <button
              ...
              className={`px-4 py-2 rounded ${
                activeTab === category ? 'bg-gray-300' : 'bg-gray-100 hover:bg-gray-200'
              }`}
